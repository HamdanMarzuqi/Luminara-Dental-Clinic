...
import { navLinks, siteInfo } from '../data/duwaData';
    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between gap-4">
          ...
          {/* Mobile Right Controls — only hamburger (CTA removed; floating WhatsApp covers it) */}
          <div className="flex items-center md:hidden">
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="p-2 rounded-lg text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-pink-500"
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-white border-b border-slate-100 shadow-xl px-4 pt-3 pb-6 space-y-1 animate-fadeIn">
          {currentNavLinks.map((link, idx) => (
            <div key={idx}>
              {link.dropdown ? (
                <div>
